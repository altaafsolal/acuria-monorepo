...
router.get('/fcc', asyncHandler(async (req, res) => {
  ...
  if (filter && filter !== 'all') {
    if (filter === 'non_envoye') clients = all.filter((c) => !c.fcc_statut || c.fcc_statut === 'Non envoyé');
    else if (filter === 'envoye') clients = all.filter((c) => c.fcc_statut === 'Envoyé' || c.fcc_statut === 'DocuSign envoyé');
    else if (filter === 'signe') clients = all.filter((c) => c.fcc_statut === 'Signé');
    else if (filter === 'renouveler') clients = all.filter((c) => c.fcc_statut === 'À renouveler');
  }
